...
CREATE OR REPLACE PACKAGE BODY QUIZ_COMPETITION_SYSTEM AS
	...
	IS
	BEGIN
		FOR P IN (SELECT * FROM Score WHERE QID = QUESTION_ID) LOOP
...
CREATE OR REPLACE PACKAGE BODY QUIZ_COMPETITION_SYSTEM AS
	...
	IS
	BEGIN
		FOR M IN (SELECT * FROM (SELECT COUNT(QID) AS NUM_OF_QUES, RID FROM Question_Gallery2 GROUP BY RID) WHERE RID = RID_) LOOP
	...
	IS
	BEGIN
		FOR N IN (SELECT * FROM (SELECT SUM(MARKS) AS TOTAL_MARKS, RID FROM Question_Gallery2 GROUP BY RID) WHERE RID = RID_) LOOP
...
CREATE OR REPLACE PACKAGE BODY QUIZ_COMPETITION_SYSTEM AS
	...
	IS
	BEGIN
		FOR R IN (SELECT * FROM Round_info NATURAl JOIN (SELECT COUNT(QID) AS NUM_OF_QUES, RID FROM Question_Gallery2 GROUP BY RID ORDER BY NUM_OF_QUES ASC)) LOOP
			ROUND_ID := R.RID;
	...
	BEGIN
		...
		FOR Q IN (SELECT * FROM Question_Gallery2 WHERE RID = ROUND_ID) LOOP
...
CREATE OR REPLACE PACKAGE BODY QUIZ_COMPETITION_SYSTEM AS
	...
	IS
	BEGIN
		FOR R IN (SELECT * FROM Round_info NATURAl JOIN (SELECT COUNT(QID) AS NUM_OF_QUES, RID FROM Question_Gallery2 GROUP BY RID)) LOOP
			ROUND_ID := R.RID;
			EXIT;
		END LOOP;
	END ROUND_SELECTED;
	...
	IS
	BEGIN
		FOR Q IN (SELECT * FROM Question_Gallery2 WHERE QID = QUESTION_ID) LOOP
			QUESTION := Q.QUESTION;
			MARKS := Q.MARKS;
...
BEGIN
	...
	INSERT INTO Asking2 VALUES(QUESTION_ID, MARKS, QUESTION);	
	DELETE FROM Question_Gallery2 WHERE QID = QUESTION_ID;
